...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <div class="nav-item">
                <a href="{{ route('guru.comments') }}" class="nav-link active">
    ...
    <div class="main-content">
        ...
        <div class="guru-card">
            <h4 class="fw-bold mb-4" style="color: var(--text-primary);"><i class="fas fa-comments text-info me-2"></i>Komentar pada Artikel Saya</h4>
            ...
            @if($comments->count() > 0)
                ...
                <div class="comment-card">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div>
                            <h6 class="fw-bold mb-1" style="color: var(--text-primary);">{{ $comment->user_nama }}</h6>
                            <small style="color: var(--text-secondary);">{{ $comment->created_at->format('d/m/Y H:i') }}</small>
                        </div>
                        <span class="badge bg-info">Komentar</span>
                    </div>
                    
                    <div class="mb-3">
                        <strong style="color: var(--text-primary);">Artikel: {{ Str::limit($comment->judul, 50) }}</strong>
                    </div>
                    
                    <div class="p-3 rounded" style="background: var(--border-color);">
                        <p class="mb-0" style="color: var(--text-secondary);">{{ $comment->isi }}</p>
                    </div>
                    
                    <div class="mt-3">
                        <a href="{{ route('articles.show', $comment->id_artikel) }}" class="btn btn-sm btn-outline-primary" target="_blank">
            ...
            @else
                <div class="text-center py-5">
                    <i class="fas fa-comments fa-4x mb-3" style="color: var(--border-color);"></i>
                    <h5 style="color: var(--text-secondary);">Belum ada komentar</h5>
                    <p style="color: var(--text-secondary);">Komentar pada artikel Anda akan muncul di sini.</p>
                </div>
            @endif
        </div>
    </div>
